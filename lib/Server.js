...
Server.prototype.onConnection = function(api, client) {
    ...
    if(this.auth !== null) {
        client.on('data', function authListener (data) {
            parser.chunk(this, chunk, function(data) {
                if(data.indexOf("AUTH:") == 0) {
                    self.auth(data.split(4), function(err, success, cb) {
                        if(err) {
                            throw new Error(err);
                        }
                        if(!success) {
                            ...
                            self.setClient({
                                ...
                                client.removeListener('data', authListener);
