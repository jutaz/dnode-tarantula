...
var Client = function(client, server, api, store, log) {
    var self        = this;
    ...
    api.$.remote    = this._remote.bind(this);
...
Client.prototype._remote = function(serverId, method) {
    args = Array.prototype.slice.call(arguments, 2);
    if(serverId === this.id) {
        this.remote[method].apply(undefined, args);
    } else {
...
var Server = function(api, options) {
    ...
    this.logging        = options.log || true;
    this.log            = new logger(this.logging);
...
Server.prototype.onConnection = function(api, client) {
    ...
    client = new Client(client, this, api, this.store, this.log);
...
Server.prototype.update = function(id) {
    ...
    this.log.debug("pushing update to remote");
}
